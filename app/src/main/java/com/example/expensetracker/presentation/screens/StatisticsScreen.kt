...
fun StatisticsScreen(
    ...
    val start = remember { DateUtils.getStartOfMonth(now) }
    val end = remember { DateUtils.getEndOfMonth(now) }

    // Allow selecting date range later (optional)
    LaunchedEffect(start, end) {
        viewModel.loadStatisticsForPeriod(start, end)
    }
